...
(function() {
	...
	if (isWindows){
	   ...
	   $('.sidebar .sidebar-wrapper .nav, .main-panel').perfectScrollbar();
   ...
   $('.sidebar-wrapper .nav >li').on('mouseover', function() {
		    ...
		    var $menuItem = $(this),
		     ...
		     var menuItemPos = $menuItem.position();
				 if(menuItemPos.top>=$('.sidebar-wrapper').height()/2) {
					 // position the submenu upward from main menu item if there is more space upwards.
					 $submenuWrapper.css({
						 bottom: $('.sidebar-wrapper').height()-(menuItemPos.top + $menuItem.height()),
					 ...
					 // position the submenu downward from main menu item if there is more space downwards.
					 $submenuWrapper.css({
						 top: menuItemPos.top,
						 left: menuItemPos.left + Math.round($menuItem.outerWidth() * 0.75),
						 bottom: "auto"
	...
	var pdp = {
		...
		initSidebarMenu: function(){
			...
			if(!mobile_menu_initialized){
				...
				nav_content = '<nav class="navbar"><ul class="nav navbar-nav nav-mobile-menu flex-column">' + nav_content + '</ul></nav>';
		  ...
		  initBootstrapNavbarMenu: debounce(function(){
			...
			if(!bootstrap_nav_initialized){
				...
				nav_content = '<ul class="nav nav-mobile-menu flex-column">' + nav_content + '</ul>';
